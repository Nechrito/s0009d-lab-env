...
bool SceneLoader::LoadModel(tinygltf::Model &model, const char *fileName) 
{
    tinygltf::TinyGLTF loader;
    string err, warn;
    auto isBinary = CString::FileType(fileName) == ".glb" ? true : false;
    return isBinary
        ? loader.LoadBinaryFromFile(&model, &err, &warn, fileName) 
        : loader.LoadASCIIFromFile(&model, &err, &warn, fileName);
}

map<int, unsigned int> SceneLoader::BindMesh(map<int, unsigned int> vbos, tinygltf::Model &model, tinygltf::Mesh &mesh) 
{
...
void SceneLoader::BindModelNodes(map<int, unsigned int> vbos, tinygltf::Model &model, tinygltf::Node &node) 
{
...
unsigned int SceneLoader::BindModel(tinygltf::Model &model) 
{
...
void SceneLoader::DrawMesh(tinygltf::Model &model, tinygltf::Mesh &mesh) 
{
    for (size_t i = 0; i < mesh.primitives.size(); ++i)
    {
        tinygltf::Primitive primitive = mesh.primitives[i];
...
void SceneLoader::DrawModelNodes(tinygltf::Model &model, tinygltf::Node &node) 
{
    if (node.mesh >= 0 && node.mesh < model.meshes.size())
        {
...
void SceneLoader::DrawModel(unsigned int vao, tinygltf::Model &model) 
{
    glBindVertexArray(vao);
